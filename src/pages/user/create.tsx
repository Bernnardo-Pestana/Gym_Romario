...
import { GET, POST, PUT, setBearerToken, setBearerTokenAdm } from '../../services/api'
...
const UpdateUser: FC<any> = () => {
    ...
    const updateUser = async () => {
        ...
        if (!name || !surname  || !age || !email || !postCode || !city || !addressLine2 || !addressLine1) {
            Swal.fire({
                title: 'Error!',
            ...
            try {
                ...
                const response = await PUT(`users/${loginInfo.id}`, {
                    "name": name,
                    "surname": surname,
                    "birthdate": age,
                    "email": email,
    ...
    const getUser = async ()=>{
        try {
            const response = await GET(`users/${loginInfo.id}`)

           const {name,email,postCode,surname,city,addressLine1,addressLine2,birthdate} = response
           setName(name)
           setEmail(email)
                ...
                <div className='card  w-100 mt-5'>
                    <div className='card-header w-100 bg-dark'>
                        <h3 className='card-title align-items-start flex-column'>
                            <span className='card-label fw-bold fs-4 mb-1 text-md-start text-white '>Hello {loginInfo.name}</span>
                        </h3>
                    </div>
